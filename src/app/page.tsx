...
export default function ElevateCVApp() {
  ...
  const [isEditingCv2, setIsEditingCv2] = useState(false);
...
Tulis ringkasan yang menarik, profesional, dan berorientasi hasil. JANGAN tulis bagian lain seperti Pendidikan atau Pengalaman. Kembalikan HANYA teks ringkasan.`;
  ...
  const handleSummarizeCv2 = async () => {
    ...
    try {
      const prompt = `Anda adalah editor CV profesional. Ringkaskan teks CV berikut menjadi lebih singkat, padat, dan profesional (maksimal 200 kata), tetapi tetap mempertahankan semua informasi penting (nama, pendidikan, pengalaman, skill).
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8">
        <header className="text-center mb-8 space-y-2">
          ...
          <p className="text-neutral-400 text-sm sm:text-base max-w-2xl mx-auto leading-relaxed">
            Platform AI untuk membuat Surat Lamaran, Analisis Skor ATS, dan Simulasi Interview.
          </p>
        </header>
          ...
          <div className="space-y-6">
            <div className="bg-neutral-900/40 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-4">
                <button 
                  onClick={() => setIsEditingCv2(!isEditingCv2)}
                  className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl shadow-lg transition-all"
                >
                  {isEditingCv2 ? "✅ Selesai Edit" : "✏️ Edit Teks"}
                </button>

                <button 
                  onClick={handleSummarizeCv2}
                  disabled={isGenCv2 || !cv2Result}
                  className="flex-1 py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl shadow-lg transition-all disabled:opacity-50"
                >
                  {isGenCv2 ? "⏳ Meringkaskan..." : "⚡ Ringkaskan dengan AI"}
                </button>

                <button 
                  onClick={exportCv2PDF} 
                  disabled={!cv2Result}
                  className="flex-1 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-bold rounded-xl shadow-lg transition-all disabled:opacity-50"
                >
                  {cv2Result ? "📥 Unduh PDF" : "⏳ Generate dulu"}
                </button>
              </div>
                ...
                <div id="cv2-preview-container" className="bg-white rounded-xl p-0 min-h-[500px] text-black overflow-hidden shadow-lg relative">
                  {cv2Result ? (
                    <div className="flex flex-row h-full min-h-[500px] font-serif text-sm text-neutral-800">
                      {/* SIDEBAR KIRI */}
                      <div className="w-[30%] bg-[#D9C8B0] p-4 flex flex-col items-center min-h-full cv2-sidebar">
                      ...
                      {/* KONTEN KANAN */}
                      <div className="w-[70%] p-6 bg-white overflow-y-auto max-h-[500px] cv2-main">
                        <h1 className="text-3xl font-bold text-neutral-700 mb-1 cv2-name">{cv2Name || "Nama Anda"}</h1>
                        <div 
                          className="text-xs text-neutral-600 leading-relaxed mb-4"
                          contentEditable={isEditingCv2}
                        ...
                        <div className="border-t border-neutral-300/50 pt-3 mb-3">
                          ...
                          <div 
                            className="text-xs text-neutral-700 whitespace-pre-line"
                            contentEditable={isEditingCv2}
                            suppressContentEditableWarning={true}
                          >
                            {cv2Edu || "Belum diisi"}
